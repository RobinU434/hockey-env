...
class HockeyPettingZooEnv(AECEnv):
    ...
    def __init__(
        ...
        # Episode tracking
        self._cumulative_rewards = dict.fromkeys(self.possible_agents, 0)
        ...
        self.infos = dict.fromkeys(self.possible_agents, {})
        self._obs = None
        self.num_moves = 0
    ...
    def step(self, action: np.ndarray):
        ...
        combined_action = np.concatenate(
            [self._actions["player_1"], self._actions["player_2"]]
        )
...
class HockeyRLlibEnv(RLlibMultiAgentEnv):
    ...
    def reset(
        ...
        observations = {"player_1": obs, "player_2": self.env.obs_agent_two()}
    ...
    def step(self, action_dict: Dict[str, np.ndarray]) -> tuple:
        ...
        # Combine actions for both players
        combined_action = np.concatenate(
            [action_dict["player_1"], action_dict["player_2"]]
        )
        ...
        rewards = {"player_1": float(reward), "player_2": float(-reward)}

        # Terminations (goal scored)
        terminateds = {"__all__": done}

        # Truncations (max steps reached)
        truncateds = {"__all__": trunc}

        # Infos
        infos = {"player_1": info, "player_2": info}
